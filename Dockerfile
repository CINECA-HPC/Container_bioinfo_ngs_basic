...
RUN apt-get install -y --no-install-recommends nano pigz wget
...
RUN . /etc/profile && cutadapt --version && snakemake --version && bedtools --version && samtools --version && bcftools --version && multiqc --version

#&& fastqc --version && multiqc --version
